...
function TigerDataMark({ size = 32 }: { size?: number }) {
  return (
    <svg
      width={size}
      height={size}
...
export default function Header({ userEmail, onSignOut }: HeaderProps) {
  ...
  return (
    <header className="bg-[#0a0a0a] text-white shadow-lg border-b border-[#222]">
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <TigerDataMark size={34} />
          <div>
            <h1 className="text-xl font-bold tracking-tight">Tiger Track</h1>
            <p className="text-xs text-gray-400">Job Leveling by Tiger Data</p>
          </div>
        </div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              <div
                ...
                style={{ backgroundColor: "rgba(245, 255, 128, 0.15)", color: "#F5FF80" }}
              >
                {displayName?.[0] || "?"}
              </div>
